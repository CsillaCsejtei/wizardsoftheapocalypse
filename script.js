...
async function fetchLocationCoordinates(locationInput) {
    ...
    try {
        const cityQueryUrl = `${GEO_DIRECT_API_URL}?q=${encodeURIComponent(locationInput)}&limit=1&appid=${API_KEY}`;
    ...
    try {
        const zipQueryUrl = `${GEO_ZIP_API_URL}?zip=${encodeURIComponent(locationInput)},${countryCode}&appid=${API_KEY}`;
...
async function handleWeatherRequest() {
    ...
    try {
        ...
        if (!locationInfo) {
            ...
            showError(`Location "${inputLocation}" not found. Please check spelling or try a different city/postcode (e.g., "London", "E14").`);
            loadingIndicator.classList.add('d-none');
            fetchWeatherBtn.disabled = false;
            return;
        }
...
async function fetchWeatherData(latitude, longitude, cityName, country) {
    ...
    try {
        ...
        if (!response.ok) {
            ...
            const errorMessageDetail = errorData.message || response.statusText || 'Unknown error';
            throw new Error(`Weather data fetch failed: HTTP error! Status: ${response.status} - ${errorMessageDetail}`);
        }
        ...
        if (data && data.main && data.weather) {
            ...
            const precipitationProbability = 'Not available from current weather endpoint'; // Still not from current weather
